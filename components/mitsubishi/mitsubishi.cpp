...
bool MitsubishiClimate::on_receive(remote_base::RemoteReceiveData data) {
  ...
  // 打印接收到的原始数据用于调试
  ESP_LOGD(TAG, "received: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
           state_frame[0], state_frame[1], state_frame[2], state_frame[3],
           state_frame[4], state_frame[5], state_frame[6], state_frame[7],
           state_frame[8], state_frame[9], state_frame[10], state_frame[11],
  ...
  if (state_frame[5] == MITSUBISHI_OFF) {
    ...
    ESP_LOGI(TAG, "off: %02X, %02X", state_frame[5], MITSUBISHI_OFF);
  } else {
    switch (state_frame[6] & 0x38) {
